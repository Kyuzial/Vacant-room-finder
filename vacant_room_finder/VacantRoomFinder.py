...
def find_all_rooms(moment, batiment):
    ...
    for floor in batiment:
        floor_output = []
        for room in floor:
            empty, until = check_if_empty(
                room, moment
            )  # until = data du créneau qui occupe la salle
            if empty:
...
def match_datetime(moment):
    ...
    try:
        match = re.compile(
            ...
            moment
        )  # Search is used otherwise, match can't find it if garbage chars surrounding date.
        if match:
            return datetime.strptime(
                f"{datetime.now().year}_{moment[match.start():match.end()]}",
                "%Y_%m-%d %H:%M",
            )
        ...
        if match:
            return datetime.strptime(moment[match.start() : match.end()], "%Y-%m-%d")
        ...
        if match:
            return datetime.strptime(
                f"{datetime.now().year}_{moment[match.start():match.end()]}", "%Y_%m-%d"
            )
...
class FreeRoomFinder(commands.Cog):
    ...
    async def find_room(self, ctx, *moments):
        moment = " ".join(
            moments
        )  # Pour accepter les formats en deux parties (ex 10/04 11h40)
        ...
        if not moment:
            ...
            if not res:
                await ctx.send(
                    "Désolé, mais la date n'a pas été reconnue.\n**Syntaxe:** `:findroom [mois/jour heure:minutes]`"
                )
                return
            else:
                moment = res
        ...
        if moment.date().isoweekday == 7:
            await ctx.send(
                f"L'université est fermée le Dimanche, aucune salle ne peut être disponible {discord_moment2}.\n**Syntaxe:** `:findroom [mois/jour heure:minutes]`"
            )
            return
            ...
            for i, floor in enumerate(output_g):
                if len(floor):
                    em_name = (
                        f"🟣 Germain — Étage {i}"
                        if i > 0
                        else "🟣 Germain — Rez-de-chaussée"
                    )
                    em_value = ""
                    for room in floor:
                        if room[1]:
                            room[1]["debut"] = "{:0>2d}:{:0>2d}".format(
                                room[1]["debut"].hour, room[1]["debut"].minute
                            )
